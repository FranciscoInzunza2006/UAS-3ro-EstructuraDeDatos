                                                ...
                                                const BenchmarkResults& results) const
{
    ...
    results_table << COLOR_MAIN << std::left << std::setw(CELL_WIDTH) << algorithm_name;

    // Sample size header
    results_table << COLOR_SECONDARY << std::right;
    for (const std::size_t& sample_size : sample_sizes)
    {
        results_table << std::setw(CELL_WIDTH) << sample_size;
    }
    results_table << COLOR_SPECIAL << std::setw(CELL_WIDTH) << "Average" << "\n";

    // Samples
    for (std::size_t type = 0; type < sample_generators_names.size(); type++)
    {
        results_table << COLOR_SECONDARY << std::setw(CELL_WIDTH) << std::left << sample_generators_names[type];

        // Benchmark result
        results_table << COLOR_DATA << std::right;
        ...
        // Type average
        results_table << COLOR_SPECIAL_DATA << std::setw(CELL_WIDTH) << results[type].back().count() << "\n";
    }

    // Size average
    results_table << COLOR_SPECIAL << std::left << std::setw(CELL_WIDTH) << "Average" << std::right <<
        COLOR_SPECIAL_DATA;
