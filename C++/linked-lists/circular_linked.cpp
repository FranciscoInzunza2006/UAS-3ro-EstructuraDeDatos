...
int main() {
	...
	do {
		...
		constexpr auto SEPARATOR =
			"-------------------------------------------------\n";
		std::cout << SEPARATOR;
		std::cout << "Menu de listas enlazadas singulares circulares\n";
		std::cout << SEPARATOR;
		displayList();
		std::cout << SEPARATOR;
...
void pushFirst() {
	...
	Node* ptr = head->next;
	while (ptr != nullptr && ptr->next != previous_head) {
		ptr = ptr->next;
	}
	ptr->next = head;
...
void pushAt() {
	...
	int i = 0;
	while (ptr != nullptr && ptr->next != head && i < target_position) {
		previous = ptr;
		ptr = ptr->next;
		i++;
	}
...
void popAt() {
	...
	Node* previous = nullptr;
	int i = 0;
	while (ptr->next != head && i < target_position) {
		previous = ptr;
		ptr = ptr->next;
		i++;
	}
